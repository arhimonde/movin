...
The Vite/React screen in `web/` consumes the normalized Roboflow fixture and presents Spanish, mobile-first inventory review. It supports loading, empty-room, review badges, quantity editing, manual additions through the 🧊 dimensions control, deletion with confirmation and undo, live total recalculation, and confirmation. Users enter length × width × height in cm; the conversion is `m³ = cm³ / 1,000,000`. For example, `100 × 50 × 40 cm = 200,000 cm³ = 0.20 m³`. The current confirmation is local UI state; production should POST the edited inventory to an API and persist an audit record.
...
- Add an object with the 🧊 control, e.g. `Tetera` / `20 × 20 × 50 cm` (the total should increase by `0.02 m³`).
- Try leaving one dimension blank or zero and verify the object cannot be submitted.
